...
public class ValidateFunction implements Function<Request, Response> {
    ...
    private ValidateResponse applyValidateRequest(ValidateRequest validateRequest) {
        ...
        if (lastAirport.getCode().equals(nextAirport.getCode())) {
            return new ValidateResponse(true, "Transaction performed from the same location");
        }

        // Time
        double minutes = Duration.between(user.getLastCardUseTimestamp(), validateRequest.getTransactionTimestamp()).toMinutes();
